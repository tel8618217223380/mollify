<?php
	class MySQLConfigurationProvider extends ConfigurationProvider {
		...
		public function getUserRootDirectories($userId) {
			$rows = $this->db->(sprintf("SELECT folder.id, user_folder.name, folder.name as folder_name, folder.path FROM user_folder, folder WHERE user_id='%s' AND folder.id = user_folder.folder_id", $this->db->string($user_id)))->rows();
